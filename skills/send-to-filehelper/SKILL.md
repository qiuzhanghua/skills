...
托盘）；建议**先手动打开「文件传输助手」会话**（macOS 键盘模式下再加 `--no-switch`，就完全不触发
微信搜索面板）；桌面必须解锁；**运行期间不要动鼠标键盘**。详见
「[Windows 后端](#windows-后端自研-rpa--built-in-rpa-backend)」一节。
...
| Input mode | 剪贴板 + 合成键鼠（会话标题 OCR 硬校验） | 辅助功能模式（可校验）或键盘模式（微信 4.1.x 实测走这条，**首选 `--no-switch`**） |
...
| **键盘模式**（`--input-mode keystrokes`） | 首选 `--no-switch`：粘贴内容 → 回车；需要换会话时才 `Esc` → `Cmd+F` 搜索 | **无**：发送前后都无法校验 |

`--input-mode auto`（默认）会先探测 AX 内容，可用就用辅助功能模式，否则自动落到键盘模式并
明确提示。键盘模式读不到当前会话，因此标准用法是**先手动打开目标会话，再加 `--no-switch`**
（不按 `Cmd+F`，微信搜索面板完全不出现）；做不到就让脚本搜索切换（默认行为）。键盘模式下
目标名称必须完全正确，默认的「文件传输助手」最安全。
...
# macOS 键盘模式首选：先手动打开目标会话，再加 --no-switch（不按 Cmd+F，搜索面板完全不出现）
...
3. 默认目标固定为「文件传输助手」；
4. **macOS 键盘模式（`--check` 会说明当前是哪种模式）首选 `--no-switch`**：先请用户在微信里停在
   目标会话，再加这个开关——不按 `Cmd+F`，搜索面板完全不出现。键盘模式读不到当前会话，
   「现在停在哪个会话」只能由人确认；会话没打开、或要发给别人时不要加它，让脚本搜索切换；
5. **只有用户明确要求发给某个人/群时才用 `--to`，并先把解析到的名称回显给用户确认**；
6. 执行后检查退出码与校验结果；
7. 失败时按「Troubleshooting」排查，不要盲目重试（可能产生重复文件消息）。
...
| `--no-switch` | 关闭 | 不切换会话，直接发给当前已打开的会话；**完全不触发微信搜索面板**。macOS 键盘模式的**首选用法**，前提是目标会话已在微信里打开 |
...
**键盘模式**（微信 4.1.x 实测路径）只依赖「辅助功能」权限发送合成键鼠事件，不依赖任何 AX 标识，
按「要不要换会话」分两条分支：

| 分支 | 动作 | 微信搜索面板 | 校验 |
| --- | --- | --- | --- |
| **首选：`--no-switch`**（目标会话已经在微信里打开） | `Cmd+V` 粘贴内容 → 回车 | **完全不出现**（不按 `Cmd+F`） | 无 |
| 需要换会话（默认） | `Esc` → `Cmd+F` → `Cmd+V` 粘贴目标名 → 回车 → 粘贴内容 → 回车 | 会弹出，脚本侧无法屏蔽 | 无 |

键盘模式的标准用法就是第一条——**先手动打开「文件传输助手」（或任何目标会话），再加
`--no-switch`**：
...
脚本不切换会话、不按 `Cmd+F`，直接粘贴发送，全程没有任何搜索面板。代价是键盘模式下**无法校验**
（读不到当前会话，`verify_note` 会写明"不做校验"，退出码仍为 0），所以这个开关只在目标会话
**确实已经打开**时使用；会话没打开就用默认的切换分支，或改用辅助功能模式（`--input-mode ax`，
它会读 `big_title_line_h_view` 校验当前会话）。

搜索面板是微信客户端自己画的结果面板（含「文件传输助手」以及并列的介绍/推荐条目，看起来像
广告），脚本改不了它、只能绕开；一定要走切换分支时，可用 `--search-delay 0.2` 缩短它停留的
时间（太小可能搜索还没出结果，回车落空）。
...
| macOS `--check` 提示「键盘模式」 | 正常现象：微信 4.1.x 不暴露聊天界面给辅助功能。**首选**先手动打开目标会话再加 `--no-switch`（不弹搜索面板）；不加则自动走 `Cmd+F` 搜索切换。此模式**不做校验**，目标名要写对 |
...
| 每次调用弹出微信搜索面板（含「文件传输助手」并列的介绍/推荐条目） | 那是 macOS 键盘模式在换会话（`Cmd+F`），脚本侧无法屏蔽。**改用首选方式**：先手动打开目标会话，再加 `--no-switch` 发送，就完全不触发；一定要换会话时，可用 `--search-delay 0.2` 缩短它显示的时间 |
...
8. macOS 键盘模式（微信 4.1.x 默认路径）**没有任何校验**：换会话那条分支（`Esc` → `Cmd+F` → 粘贴
   名称 → 回车）如果名称在微信里查不到，微信不会打开会话，随后粘贴的文件/文字会发给**当前已打开
   的那个会话**。所以键盘模式下务必核对 `--to`；首选做法是**先把目标会话打开再用 `--no-switch`**，
   此时发往哪个会话完全由用户打开的那个会话决定，不依赖名称匹配。
