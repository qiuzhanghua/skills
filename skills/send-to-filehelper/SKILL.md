...
```bash
# 推荐带上 --no-verify：跳过**发送后**的 OCR 结果复核（客户端认错字时它永远通不过，
# 每次白等约 17s）。发送前的会话校验与输入框确认都保留。
...
- **建议加 `--no-verify`**：它只跳过**发送后**的结果复核（自研后端为 `_text_present` + 上层 `verify_sent`）。OCR 除了滞后还会**认错字**（实测 `性`->`陸`、`3`->`引`），这种错误每帧一致、重试无效，于是发送后复核永远通不过却每次白等约 17s。
  发送**前**的两道 OCR 都保留：会话识别与硬校验（用不存在的会话名测过，仍 exit=1 拒绝发送）、以及输入框确认。发错人的安全底线不变。
...
0. **预检**（纯 stdlib）：枚举进程与顶层窗口，确认微信在运行、读客户端文件版本、必要时还原最小化
   的主窗口；没有微信 / 主窗口在托盘时直接给出结论；
1. `WinWeChat()` 找到主窗口（按面积挑最大的 Qt 窗口，并用 OCR 打分排除停在扫码登录页的第二个
   微信进程），必要时把窗口收成完整可见、足够宽的尺寸；
2. `wx.ChatWith(target, exact=True)`：OCR 读会话列表 → 精确匹配 → 否则唯一前缀匹配（适配被截断的
   「文件传...」）→ 点击 → 再 OCR 复核聊天区标题；
3. `wx.ChatInfo()['chat_name']` 与目标比对，不一致即中止并列出候选会话；
4. 文本用 `wx.SendMsg(text)`：聚焦输入框 → 剪贴板 + `Ctrl+V` → 确认文字进了输入框 → `Enter`；
5. 文件用 `wx.SendFiles([...绝对路径...])`：像素分析定位工具栏「发送文件」图标 → 系统文件对话框
   填完整路径 → 回车 → 把主窗口置前 → **点界面上的「发送」按钮**（附件挂上后回车提交不了）；
6. `wx.GetAllMessage()` 做发送后校验：OCR 多帧取并集，按文件名 / 文本内容匹配。

后端的返回值与「显式成功/无法判定」的归一化处理见 `describe_result()`；校验不到**不算失败**
...
| 消息含标点/数字时总报「未能复核」，但其实已经发出 | OCR 会把标点读成异体（`【`→`〖`/`〔`）、把 `16:25:27` 读成 `16 ： 25 ： 27`，并把长消息折成多行。校验现在会先做归一化（NFKC + 标点归并 + 去空格）再比对全文，这类误报已大幅减少；仍复核不到时以微信界面为准 |
| 发送完成后还要等十几秒才结束 | 那是发送后的 OCR 结果复核在空转（客户端认错字时永不通过）。加 `--no-verify` 跳过它；发送前的会话校验与输入框确认仍在 |
